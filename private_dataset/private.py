...
#             'train': {'month': self.month[:13], 'date': self.date[:13]},
#             'test': {'month': self.month[-31:], 'date': self.date[-31:]}
            'train': {'month': self.month[:1], 'date': self.date[:1]},
            'test': {'month': self.month[-1:], 'date': self.date[-1:]}
    ...
    # tshark -r 20240528.pcap -o 'uat:zigbee_pc_keys:"5A6967426565416C6C69616E63653039","Normal",""' -T fields -E separator=$ -e wpan.cmd > output.csv
    def run_tshark(self):
        base_dir = os.getcwd()
        command = 'tshark -r {}/silent-test/pcap/{}.pcap -T fields -E separator=$ -e frame.number -e frame.time_epoch '\
                  ...
                  'tcp.dstport -e tcp.stream -e tcp.window_size -e tcp.len ' \
                  '-e udp.srcport -e udp.dstport -e udp.stream -e dns.qry.name -e http -e ntp >{}/silent-test/csv/{}.csv'
