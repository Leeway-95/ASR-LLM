...
public class LogicalShapelets extends BaseShapelets {
    ...
    public static final int MAXREF = 5;
    public int stepSize = 1;

    public LogicalShapelets(SequenceDataset trainSet, int minLen, int maxLen, int stepSize) {
...
//        System.out.format("Bhool Count %d\n\n\n",bhulCount);
        Sequence shapeletTS = new Sequence(trainSet.get(bestSplit.shapeletID), bestSplit.shapeletPos, bestSplit.shapeletPos + bestSplit.shapeletLength);
...
//	        c = (c < 0.0) ? 0 : Math.sqrt(2*len*(1-c));
    ...
    class OrderLine {
        ...
        public Double leftTotal = 0D;
        public Double rightTotal = 0D;
        public Double leftEntropy = 0D;
        public Double rightEntropy = 0D;

        public Double informationGain = 0D;
        public Double gap = 0D;
        public Double splitDist = -1D;
        ...
        public OrderLine(SequenceDataset trainSet) {
            N = trainSet.size();
        ...
        public Double shiftEntropy(Double shiftAmount) {
            ...
            Double maxInf = 0D, maxGap = 0D, maxDistance = 0D;
            int maxi = -1;
        ...
        public Double TUB(Double shiftAmount) {
            ...
            leftTotal = 0D;
            rightTotal = Double.parseDouble(N + "");
            ...
            for (k = 0; k < N - 1; k++) {
                ...
                leftTotal = Double.parseDouble(ltot_tmp + "");
                rightTotal = Double.parseDouble(rtot_tmp + "");
        ...
        public Double computeInformationGain() {
            ...
            leftEntropy = 0D;
            for (Integer i : clsHist.keySet()) {
                ratio = Double.parseDouble(leftClsHist.get(i) + "") / leftTotal;
                if (ratio > 0)
            ...
            rightEntropy = 0D;
            for (Integer i : clsHist.keySet()) {
                ratio = Double.parseDouble(rightClsHist.get(i) + "") / rightTotal;
                if (ratio > 0)
