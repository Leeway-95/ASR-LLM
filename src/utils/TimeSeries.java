...
//               timeValueMap.put(timeReadings.get(timeReadings.size()-1), readings);
    ...
    private static boolean determineIsFirstColTime(String filePath) {
        ...
        try {
            ...
            //    'equalityFlexibilityFactor') between all values in
            //    'possibleTimeValues'.
...
//         outStr.append(timeReadings.get(r).toString());
    ...
    public ArrayList getTsArray() {
        return tsArray;
    }

    public Double[] toTsArray() {
        Double[] ts = new Double[tsArray.size()];
        for (int i = 0; i < tsArray.size(); i++) {
            ts[i] = (Double) tsArray.get(i);
        }
        return ts;
    }

    public ArrayList getTimeReadings() {
        return timeReadings;
    }
